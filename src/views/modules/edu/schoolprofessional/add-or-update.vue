	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
			...
			<el-form-item label="专业名称" prop="professionalName">
				<el-input v-model="dataForm.professionalName" placeholder="专业名称" style="width:60%"></el-input>
			</el-form-item>
			<el-form-item label="专业代号" prop="professionalCode">
				<el-input v-model="dataForm.professionalCode" placeholder="专业代号" style="width:60%"></el-input>
			</el-form-item>
			<el-form-item label="专业编号" prop="professionalNumber">
				<el-row :gutter="10">
					<el-input v-model="dataForm.professionalNumber" placeholder="专业编号,输入专业名称后查询" readonly
	...
	export default {
		...
		methods: {
					...
					if (this.dataForm.id) {
						this.$http({
							...
							if (data && data.code === 0) {
								...
								this.dataForm.professionalNumber = data.schoolProfessional
									.professionalNumber
					...
					if (valid) {
						this.$http({
							url: this.$http.adornUrl(
								`/edu/schoolprofessional/${!this.dataForm.id ? 'save' : 'update'}`),
							method: 'post',
			...
			queryProfessionalNumber() {
				this.$http({
					url: this.$http.adornUrl(
						`/dictionary/name/PROFESSIONAL_CODE/${this.dataForm.professionalName}`),
					method: 'get'
				}).then(({
					data
				}) => {
